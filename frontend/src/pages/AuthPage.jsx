...
const AuthPage = () => {
    ...
    const handleGoogleSuccess = () => {
        login({identifier: normalizedEmail || null});
        navigate('/dashboard');
    };
    ...
    const showGoogleHint = step === AUTH_STEP.GOOGLE_ONLY || step === AUTH_STEP.SIGNUP_CREATE_PASSWORD || step === AUTH_STEP.PASSWORD_LOGIN;
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-slate-950 to-slate-900">
            <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-950 p-8 shadow-2xl text-white">
                ...
                <p className="mt-2 text-sm text-slate-400">Use email or continue with Google.</p>
                ...
                <div className="mt-6 space-y-3">
                    {showGoogleHint && <p className="text-sm text-slate-300">You can also continue with Google.</p>}
                    <GoogleLoginButton
