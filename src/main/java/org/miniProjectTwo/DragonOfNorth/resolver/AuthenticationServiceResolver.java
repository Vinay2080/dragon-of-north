...
public class AuthenticationServiceResolver {
    ...
    public AuthenticationService resolve(AppUserStatusFinderRequest request) {
        IdentifierType type = request.identifierType();
        if (type == EMAIL && !isMail(request)) {
            throw new BusinessException(IDENTIFIER_MISMATCH, EMAIL);
        }
        if (type == PHONE && !isPhone(request)) {
            throw new BusinessException(IDENTIFIER_MISMATCH, PHONE);
        }
        return serviceMap.get(type);
    }

    private boolean isMail(AppUserStatusFinderRequest request) {
        return request.identifier().matches("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$");
    }

    private boolean isPhone(AppUserStatusFinderRequest request) {
        return request.identifier().matches("^\\+91[6-9]\\d{9}$");
    }
}
